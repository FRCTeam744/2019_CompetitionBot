...
class OI
{
  ...
  private:
    ...
    const double FRONT_HIGH_HATCH_POSITION = 3000;//RADIANS_TO_DEGREES * acos((PIVOT_HEIGHT - HIGH_HATCH_HEIGHT) / (ARM_LENGTH));
    ...
    const double BACK_HIGH_HATCH_POSITION = -3000.0;//RADIANS_TO_DEGREES * -acos((PIVOT_HEIGHT - HIGH_HATCH_HEIGHT) / (ARM_LENGTH));
